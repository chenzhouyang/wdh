...
public class RiseNumberTextView extends TextView implements IRiseNumber {
	...
	private void runInt() {
		...
		ValueAnimator valueAnimator = ValueAnimator.ofInt(fromIntNumber,
				intNumber);
		valueAnimator.setDuration(duration);
	...
	public void withNumber(int number) {
		...
		numberType = 1;
		if (number > 1000) {
			fromIntNumber = (int)(number
					- (float) Math.pow(10, sizeOfInt(number) - 2));
		} else {
			fromIntNumber = number / 2;
		}
	}
